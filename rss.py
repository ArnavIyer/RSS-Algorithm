...
class graph:
    ...
    #         If you don't give it a tour, it will add the closest RSS to the closest end of each tour, making sure each tour is connected to its closest RSS.
    #
    # Note: Right now, this only supports having ONE central storage facility. It is very easy to add support for multiple.
    ...
    # This is just used for logic in self.checkCapacity()
    def findClosest(self,  one,  neg):
        ret = -1 if one>neg else 0
        return ret
        # if (one>neg): 
        #     return -1
        # else: 
    ...
    def makeTours(self, nodesUV=None, withCapacity=None):
        ...
        total1 = self.unloadingTime # stores total cost for each tour
        total2 = self.unloadingTime 
        pop1 = self.popList[end1]   # stores the population for each tour
        pop2 = self.popList[end2]
        tour1 = [end1]              # stores tour.
        tour2 = [end2]
        end = True
        ...
        while len(nodesUV) != 0: # While there aren't any unvisited nodes
            ...
            t1 = False  
            t2 = False
            if self.rssDist(tour1,total1) > self.rssDist(tour2,total2):	# which route is currently shorter, including the distance to the closest RSS?
                t2 = True
        ...
        rsscost = self.addRSS(tour2)        # Do the same for the other route
        total2 += rsscost
...
# Print out results for these testcases in the file
tex = open("RSSresults.txt", "w")
tex.write("Format:")
tex.write("\n\n")

tex.write("Amount of Tours")
tex.write("\n")

tex.write("Route Number")
tex.write("\n")
tex.write("[List of ordered PODs in Tour]")
tex.write("\n")
tex.write("[Total Cost of Tour, Population of Tour]")
tex.write("\n")
